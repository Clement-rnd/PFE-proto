...
function LineChart({ width }: { width: number }) {
  ...
  // dashed line via stroke-dasharray (SVG)
  const yLabels = [40, 50, 60];
  const xLabels = [0, 2, 4, 6, 7]; // Jan, Mar, Mai, Jul, Aoû
  ...
  return (
    <Svg width={width} height={CHART_H}>
      ...
      {/* Last point dot */}
      <Circle
        cx={xFor(NAYA_DATA.length - 1)}
        cy={yFor(NAYA_DATA[NAYA_DATA.length - 1])}
        r={4}
